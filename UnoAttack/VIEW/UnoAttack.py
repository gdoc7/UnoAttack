...
images = {
    ...
    "Up": ImageTk.PhotoImage(Image.open(f"{folder}/Up.png").convert("RGBA")),
    "Left": ImageTk.PhotoImage(Image.open(f"{folder}/Left.png").convert("RGBA")),
    "Right": ImageTk.PhotoImage(Image.open(f"{folder}/Right.png").convert("RGBA")),
...
cards = ["Wild", "Wild_Draw", "Wild_Attack", "Skip", "Draw", "Reverse", "Discard"] + [str(n) for n in range(10)]
...
playerPicker = {"Up": canvas.create_image(int(canvas["width"])/2+250, int(canvas["height"])/2-100,image=images["Up"], state="hidden"),
                "Left": canvas.create_image(int(canvas["width"])/2+180, int(canvas["height"])/2-30,image=images["Left"], state="hidden"), # 181, -30
                "Right": canvas.create_image(int(canvas["width"])/2+320, int(canvas["height"])/2-30,image=images["Right"], state="hidden"), # 322, -31
                }
    ...
    def use(self, player):                # Uso de carta
            ...
            if self.name == "Wild_Draw": # Wild draw 4, chequea legalidad
                ...
                if legal:
                    ...
                    for n in range(4):
                        o = getRandom()
                        player.draw(o)
            if player == game.user and len(player.hand) > 0 and self.name == "Wild_Draw":
                game.pickingColour = True
                toggleColourPicker("normal")   # Muestra los botones de cambio de color
            elif player == game.user and self.name == "Wild_Attack":
                game.pickingPlayer = True
                togglePlayerPicker("normal")
            incrementTurn = False
        ...
        if len(player.hand) == 0: # game over
            ...
            for plr in game.players:
                ...
                plr.hand = []
            game.log.output(f"{player.name} ganó, obteniendo {totalScore} puntos!")
            if player == game.user:
                game.log.output("Felicidades!")
            else:
                game.log.output("Más suerte la próxima vez!")
...
class Deck():
        ...
        for colour in ["Yellow", "Red", "Green", "Blue"]:
            ...
            for cardType in cards:
                ...
                if len(cardType) == 1: # Carta de números
                    ...
                    cardType = "Discard"
                    displayName = f"{colour} {cardType}"
                elif cardType.find("Wild_Attack") > -1:  # Cartas de ataque (doble manotazo apuntado). Crear una vez (total 4 cartas).
                    worth = 50
                    name = f"{colour}_{cardType}"
                    cardType = "Wild_Attack"
                    displayName = f"{colour} {cardType}"
    ...
    def checkUsedPile(self, time):    #Chequea la pila de cartas usadas
        if self.lastCardUsed.name == "Wild_Draw" and self.lastCardUsed.name == "Wild_Attack":
            # Wild Draw 4 = Reshuffle + new discard
            game.log.output("Carta Wild fue la primera carta en la pila. Volviendo a barajear...")
...
class Game():    
    def __init__(self):
        ...
        self.pickingPlayer = False
        self.playerCount = 2
        self.turnList = []           # Lista de turnos
        self.turnNumber = 0          # Numero de turno
        self.increment = 1           # Incremento del turno
        self.botDelay = 1000
        self.firstGame = True
        ...
        # Botones para escoger número de jugadores
        ...
        for k, widget in self.playerCountWidgets.items():
            if k != "label":
                ...
                #tag_bind es para establecer una relacion entre un elemento de interfaz con una funcion
    ...
    def real_init(self, playerCount):
        ...
        playerHighlights = ["red3","CadetBlue3", "DarkGoldenRod2", "plum3"]   # Colores de los jugadores (para el texto de Log)
        self.user = Player("Player1", 1, playerHighlights[0])
        self.players.append(self.user)
        for i in range(1, self.playerCount):        # Para cada jugador, se le aplica el color de texto (por ahora se usan bots)
        ...
        if self.playerCount == 2:      # Si hay dos jugadores
            ...
            if self.playerCount == 4:     # 4 jugadores
    ...
    def toggleUno(self, event=None):    # Cantar UNO (evento de click sobre el botón de UNO)
        ...
        if self.uno:
            self.log.output(f"Uno status ON")
        else:
            self.log.output("Uno status OFF")
    ...
    def attackPlayer(self, event, players=["Up", "Right", "Left"]):
        togglePlayerPicker("hidden")
        player = ""
        if event:
            for key, obj in playerPicker.items():
                if canvas.find_withtag(CURRENT)[0] == obj:
                    player = key
                    self.attack(player)
        else:
    ...
    def attack(self, option):
        if option == "Up":
            turn = self.increment + 1
            for i in range(2):
                ra = getRandom()
                self.turnList[game.simplifyTurnNumber(False, turn)].draw(ra)
        if option == "Right":
            turn += self.increment
            for i in range(2):
                ra = getRandom()
...
def togglePlayerPicker(newState):       # Mostrar/Esconder escoger jugador
    for k, obj in playerPicker.items():
        canvas.itemconfigure(obj, state=newState)
...
for k, obj in playerPicker.items():
    canvas.tag_bind(obj,"<1>", game.attackPlayer)
